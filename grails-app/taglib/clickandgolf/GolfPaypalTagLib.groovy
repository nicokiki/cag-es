...
class GolfPaypalTagLib {
	...
	def paypal = { attrs ->
		...
		if(!config.server || !config.email) {
			...
			out << g.form(name:"paypalFormId", controller:"paypal",action:"buy", params:formParams) { """ ${returnAction}
				    ${returnController}
				    ${cancelAction}
				    ${cancelController}
					${additionalParams}
					...
					<input id=\"imagePaypalId\" type=\"image\" onclick=\"return showMessage();\" src=\"${buttonSrc}\" border=\"0\" name=\"submit\" alt=\"${buttonAlt}\">
					<img alt="" border=\"0\" src=\"https://www.paypal.com/en_US/i/scr/pixel.gif\" width=\"1\" height=\"1\">
					""" 
			}
//			<input type=\"image\" onclick=\"return showMessage();\" src=\"${buttonSrc}\" border=\"0\" name=\"submit\" alt=\"${buttonAlt}\">
