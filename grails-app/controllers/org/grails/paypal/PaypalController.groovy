...
class PaypalController {
	...
	def notifyPaypal = {
		...
		if (payment && result == 'VERIFIED') {
			if (params.receiver_email != login) {
				log.warn """WARNING: receiver_email parameter received from PayPal does not match configured e-mail. This request is possibly fraudulent! REQUEST INFO: ${params}
				"""
			}
			else {
				...
				if (payment.status != Payment.COMPLETE && payment.status != Payment.CANCELLED) {
					if (payment.paypalTransactionId && payment.paypalTransactionId == params.txn_id) {
						log.warn """WARNING: Request tried to re-use and old PayPal transaction id. This request is possibly fraudulent! REQUEST INFO: ${params} """
					}
	...
	void updateBuyerInformation(payment, params) {
		...
		// Para evitar problemas, evito guardar toda esta info que vino desde Paypal asi respeto las condiciones
		buyerInfo.firstName = null
		buyerInfo.lastName = null
		buyerInfo.companyName = null
		buyerInfo.receiverName = null
		buyerInfo.email = null
		buyerInfo.street = null
		buyerInfo.zip = null
		buyerInfo.city = null
		buyerInfo.state = null
		buyerInfo.country = null
		buyerInfo.countryCode = null
		buyerInfo.phoneNumber = null
